...
let randomFireworksInterval: string | number | NodeJS.Timer | undefined;
...
function explodeSilent(container: HTMLElement | null, rocketEl: HTMLDivElement, color: string, explosionType: string, size: string) {
...
function launchRocketWithSound(container: HTMLElement | null, { left, color, explosionType, size }: { left: string; color: string; explosionType: string; size: string; launchTime: number; }) {
  const rocketEl = document.createElement('div');
  rocketEl.className = 'firework-rocket';
...
function explodeWithSound(container: HTMLElement | null, rocketEl: HTMLDivElement, color: string, explosionType: string, size: string) {
  const rect = rocketEl.getBoundingClientRect();
  const centerX = rect.left + rect.width / 2;
  const centerY = rect.top + rect.height / 2;
  rocketEl.remove();
...
function launchGrandFinaleRocket(container: HTMLElement | null) {
  const left = '50%';
  const color = '#FFFFFF';
...
function bigSlowExplosion(container: HTMLElement | null, rocketEl: HTMLDivElement) {
  const rect = rocketEl.getBoundingClientRect();
  const centerX = rect.left + rect.width / 2;
  const centerY = rect.top + rect.height / 2;
  rocketEl.remove();
...
function createSpark(container: HTMLElement | null, rocketEl: HTMLDivElement, color: string) {
  const rect = rocketEl.getBoundingClientRect();
  const centerX = rect.left + rect.width / 2;
  const centerY = rect.top + rect.height / 2;
...
function createFragment(container: HTMLElement | null, x: number, y: number, color: string, angle: number, size: string, magnitude: number) {
  const fragment = document.createElement('div');
  fragment.className = 'firework-fragment';
...
function getExplosionPattern(type: string) {
...
async function playFireStageSound(size: string) {
...
async function playExplosionSound(size: string) {
  if (Tone.context.state !== 'running') {
    await Tone.start();
  }
...
export { startMainFireworkShow, startRandomFireworks }